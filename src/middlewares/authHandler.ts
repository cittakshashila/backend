...
export const AuthourizeAdmin = (req : Request , res :  Response, next : NextFunction) => {
...
export const AuthourizeUser = async (req: Request, res: Response, next: NextFunction) => {
  const authHeader = req.headers['authorization']
  const token : string = authHeader && authHeader.split(' ')[1] || ""
...
export const CreateAdminToken = async (req: Request, res: Response) => {
  const tokenData: tokenType = {is_admin: true, user : {email: req.body.uname}}
  const token : string = jwt.sign(tokenData, SECRET)
  return res.send({"token": token})
}

export const CreateEventAdminToken = async (req: Request, res: Response) => {
  const tokenData : tokenAdminToken = { is_event_admin : true, event_id : req.body.event_id }
  const token : string = jwt.sign(tokenData, SECRET)
  return res.send({"token": token})
}
